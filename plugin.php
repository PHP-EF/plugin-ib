	...
	public function generateSecurityReport($StartDateTime,$EndDateTime,$Realm,$UUID,$unnamed,$substring) {
		...
		if (is_array($UserInfo) && isset($UserInfo['Error'])) {
			...
			$CubeJSRequests = array(
				...
				'DNSActivity' => '{"measures":["PortunusAggInsight.requests"],"dimensions":[],"timeDimensions":[{"dimension":"PortunusAggInsight.timestamp","dateRange":["'.$StartDimension.'","'.$EndDimension.'"],"granularity":"day"}],"filters":[{"member":"PortunusAggInsight.type","operator":"equals","values":["1"]}],"limit":"1","ungrouped":false}',
			...
			$embeddedFiles = array_diff(scandir($embeddedDirectory), array('.', '..'));

			$this->logging->writeLog("Assessment","Embedded Files List","debug",$embeddedFiles);
